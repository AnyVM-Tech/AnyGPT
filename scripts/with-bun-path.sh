...
if command -v bun >/dev/null 2>&1; then
	...
	echo "Workspace-surface operator note for thread 8bd76091-7a92-4314-abf8-926521f7bacf:workspace-surface: install Bun from https://bun.sh or add its bin directory to PATH, set BUN_INSTALL correctly, then rerun 'bash ./scripts/with-bun-path.sh bun run frontend:install-check' or another bounded workspace smoke/typecheck command for this same thread." >&2
