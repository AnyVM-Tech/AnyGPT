...
const HAS_BUN_FILE_IO =
	typeof BunRuntime?.file === 'function' &&
	typeof BunRuntime?.write === 'function';
...
async function renameFileCompat(
	sourcePath: string,
	targetPath: string
): Promise<void> {
...
// FIX: Add export
export interface ModelsFileStructure {
	object: string;
...
// --- Configuration ---
function resolveManagedDataFilePath(
	envVarName: string,
...
const filePaths: Record<DataType, string> = {
	providers: resolveManagedDataFilePath(
		'API_PROVIDERS_FILE',
		'providers.json'
	),
...
const inMemoryCache: Partial<
	Record<DataType, { value: ManagedDataStructure; ts: number }>
> = {};
...
async function backupInvalidKeysJson(
	filePath: string,
	error: unknown
): Promise<void> {
	try {
		...
		console.warn(
			`[DataManager] Backed up invalid keys.json to ${backupPath}.`
		);
	} catch (backupError) {
		console.error(
			`[DataManager] Failed to back up invalid keys.json (${filePath}):`,
			backupError
		);
	}
	console.error(
		`[DataManager] Invalid JSON in ${filePath}. Skipping overwrite to avoid data loss.`,
		error
	);
}
...
function createAtomicTmpPath(filePath: string, tmpDir: string | null): string {
	...
	return tmpDir
		? path.join(tmpDir, `${basename}.tmp-${uniqueSuffix}`)
		: `${filePath}.tmp-${uniqueSuffix}`;
}
...
async function ensureTmpDirExists(): Promise<string | null> {
	...
	try {
		...
		if (!dataTmpDirUnavailable) {
			console.error(
				`[DataManager] Failed to ensure DATA_TMP_DIR (${resolvedDataTmpDir}); falling back to in-place tmp files.`,
				err
			);
		}
...
async function writeFileAtomic(
	...
	const tmpPath = createAtomicTmpPath(filePath, tmpDir);
	try {
...
const providersFilesystemSyncIntervalMs = Math.max(
	0,
	Number(process.env.PROVIDERS_FS_SYNC_INTERVAL_MS ?? '30000') || 30000
);

const pendingFsWrites = new Map<
	DataType,
	{ timer: NodeJS.Timeout; data: string; filePath: string }
>();

function getFilesystemSyncIntervalMs(dataType: DataType): number {
	if (dataType === 'keys') return keysFilesystemSyncIntervalMs;
	if (dataType === 'providers') return providersFilesystemSyncIntervalMs;
	return 0;
}

const keyPreservationDir = path.resolve(
	(process.env.KEYS_PRESERVATION_DIR ?? '').trim() ||
		path.join(path.dirname(filePaths.keys), 'key-preservation')
);
const keyPreservationMirrorPath = path.resolve(
	(process.env.KEYS_PRESERVATION_FILE ?? '').trim() ||
		path.join(keyPreservationDir, 'keys.preserved.json')
);
const keySnapshotDir = path.resolve(
	(process.env.KEYS_SNAPSHOT_DIR ?? '').trim() ||
		path.join(keyPreservationDir, 'snapshots')
);
const keySnapshotIntervalMs = Math.max(
	0,
	Number(process.env.KEYS_SNAPSHOT_INTERVAL_MS ?? '300000') || 300000
);
const keySnapshotMaxFiles = Math.max(
	1,
	Number(process.env.KEYS_SNAPSHOT_MAX_FILES ?? '288') || 288
);
const allowDestructiveKeySync = process.env.KEYS_ALLOW_DESTRUCTIVE_SYNC === '1';
let lastKeySnapshotAt = 0;

// Prefer Redis by default when available; allow forcing filesystem via env.
const dataSourcePreference: 'redis' | 'filesystem' =
	process.env.DATA_SOURCE_PREFERENCE === 'filesystem'
		? 'filesystem'
		: 'redis';
console.log(
	`[DataManager] Data source preference set to: ${dataSourcePreference}`
);
...
const keysRedisReadTimeoutMs = Math.max(
	100,
	Number(process.env.KEYS_REDIS_READ_TIMEOUT_MS ?? redisReadTimeoutMs) ||
		redisReadTimeoutMs
);
...
class DataManager {
	...
	constructor(redisInstance: Redis | null) {
		...
		if (this.redisClient) {
			this.redisClient.on('ready', () =>
				console.log('DataManager: Redis client ready.')
			);
			this.redisClient.on('error', err =>
				console.error('DataManager: Redis client error:', err)
			);
			console.log(
				`DataManager initialized with Redis client (${this.redisClient.status}).`
			);
			this.attachRedisReadyHandler();
		} else {
			console.log(
				'DataManager initialized without Redis client (using filesystem only).'
			);
		}
	}
	...
	private async getCachedOrFilesystem<T extends ManagedDataStructure>(
		dataType: DataType
	): Promise<T | null> {
		...
		try {
			...
			if (err?.code !== 'ENOENT') {
				console.error(
					`[DataManager] Failed reading ${dataType} from filesystem for Redis backfill:`,
					err
				);
			}
			return null;
	...
	private async saveToRedisOnly<T extends ManagedDataStructure>(
		dataType: DataType,
		data: T
	): Promise<void> {
		...
		if (
			...
			console.warn(
				'[DataManager] Skipping Redis backfill for providers because data array is empty.'
			);
			return;
		}
	...
	private async backfillPendingToRedis(reason: string): Promise<void> {
		...
		this.redisBackfillPromise = (async () => {
			...
			const targets = pendingTypes.filter(
				t => t === 'models' || t === 'providers'
			);
			if (targets.length === 0) return;

			console.log(
				`[DataManager] Redis ready; backfilling pending data (${targets.join(', ')}) [${reason}]...`
			);

			if (targets.includes('models')) {
				const modelsData =
					await this.getCachedOrFilesystem<ModelsFileStructure>(
						'models'
					);
				if (modelsData) {
					await this.syncModelsMirrorIfNeeded(
						modelsData,
						'filesystem'
					);
					...
					console.warn(
						'[DataManager] No models data available for Redis backfill.'
					);
				}
			...
			if (targets.includes('providers')) {
				const providersData =
					await this.getCachedOrFilesystem<LoadedProviders>(
						'providers'
					);
				if (providersData) {
					...
					console.warn(
						'[DataManager] No providers data available for Redis backfill.'
					);
				}
	...
	private mergeKeyRecords(
		a?: Partial<UserData>,
		b?: Partial<UserData>
	): UserData {
		const fsUser =
			a && typeof a === 'object' ? (a as Record<string, any>) : {};
		const redisUser =
			b && typeof b === 'object' ? (b as Record<string, any>) : {};
		const roleA =
			a?.role === 'admin'
				? 'admin'
				: a?.role === 'user'
		...
		const toCounter = (value: unknown): number | undefined => {
		...
		const toCost = (value: unknown): number | undefined => {
			...
			return num !== undefined && num >= 0
				? Math.round(num * 1_000_000) / 1_000_000
				: undefined;
		};
		const maxDefined = (
		...
		const preferredString = (
			...
			if (typeof redisValue === 'string' && redisValue.trim())
				return redisValue;
			if (typeof fsValue === 'string' && fsValue.trim()) return fsValue;
			return fallback;
		};
		const preferredTimestamp = (
			...
			const redisTs =
				...
				Number.isFinite(Date.parse(redisValue))
					? redisValue
					: undefined;
			const fsTs =
				typeof fsValue === 'string' &&
				Number.isFinite(Date.parse(fsValue))
					? fsValue
					: undefined;
			return redisTs ?? fsTs;
		};
		...
		const merged = {
			...fsUser,
			...redisUser,
			userId: preferredString(b?.userId, a?.userId, 'unknown_user'),
			tokenUsage: Math.max(0, redisTokenUsage ?? 0, fsTokenUsage ?? 0),
			...
			tier: preferredString(b?.tier, a?.tier, 'free')
		} as Record<string, any>;

		const estimatedCost = maxDefined(
			toCost(a?.estimatedCost),
			toCost(b?.estimatedCost)
		);
		if (estimatedCost !== undefined) {
			merged.estimatedCost = estimatedCost;
		}

		const paidTokenUsage = maxDefined(
			toCounter(a?.paidTokenUsage),
			toCounter(b?.paidTokenUsage)
		);
		if (paidTokenUsage !== undefined) {
			merged.paidTokenUsage = Math.min(merged.tokenUsage, paidTokenUsage);
		}

		const paidRequestCount = maxDefined(
			toCounter(a?.paidRequestCount),
			toCounter(b?.paidRequestCount)
		);
		if (paidRequestCount !== undefined) {
			merged.paidRequestCount = Math.min(
				merged.requestCount,
				paidRequestCount
			);
		}

		const paidEstimatedCost = maxDefined(
			toCost(a?.paidEstimatedCost),
			toCost(b?.paidEstimatedCost)
		);
		if (paidEstimatedCost !== undefined) {
			merged.paidEstimatedCost = Math.min(
				merged.estimatedCost ?? 0,
				paidEstimatedCost
			);
		}

		const billingPeriodStartedAt = preferredTimestamp(
			b?.billingPeriodStartedAt,
			a?.billingPeriodStartedAt
		);
		if (billingPeriodStartedAt) {
			merged.billingPeriodStartedAt = billingPeriodStartedAt;
		}

		const lastBillingSettlementAt = preferredTimestamp(
			b?.lastBillingSettlementAt,
			a?.lastBillingSettlementAt
		);
		if (lastBillingSettlementAt) {
			merged.lastBillingSettlementAt = lastBillingSettlementAt;
		}

		if (
			typeof b?.disabled !== 'undefined' ||
		...
		const redisDailyDate =
			typeof b?.dailyRequestDate === 'string' &&
			/^\d{4}-\d{2}-\d{2}$/.test(b.dailyRequestDate)
				? b.dailyRequestDate
				: undefined;
		const fsDailyDate =
			typeof a?.dailyRequestDate === 'string' &&
			/^\d{4}-\d{2}-\d{2}$/.test(a.dailyRequestDate)
				? a.dailyRequestDate
				: undefined;
		if (redisDailyDate || fsDailyDate) {
			const dailyDate =
				redisDailyDate && fsDailyDate
					? redisDailyDate >= fsDailyDate
						? redisDailyDate
						: fsDailyDate
					: (redisDailyDate ?? fsDailyDate);
			merged.dailyRequestDate = dailyDate;
			const redisDailyCount =
				redisDailyDate === dailyDate
					? toCounter(b?.dailyRequestCount)
					: undefined;
			const fsDailyCount =
				fsDailyDate === dailyDate
					? toCounter(a?.dailyRequestCount)
					: undefined;
			merged.dailyRequestCount = Math.max(
				0,
				redisDailyCount ?? 0,
				fsDailyCount ?? 0
			);
	...
	private countKeys(keys: KeysFile | null | undefined): number {
	...
	private findMissingKeys(
		...
		for (const apiKey of Object.keys(reference || {})) {
			if (
	...
	private describeKeySet(keys: string[]): string {
	...
	private async readKeysJsonFile(
		filePath: string,
		label: string
	): Promise<KeysFile> {
		try {
			const raw = await readTextFile(filePath);
			return this.normalizeKeysSchema(JSON.parse(raw || '{}') as KeysFile)
				.normalized;
		} catch (err: any) {
			if (err?.code === 'ENOENT') return {};
			await backupInvalidKeysJson(filePath, err);
			console.error(
				`[DataManager] Failed reading ${label} keys from ${filePath}. Treating it as empty for preservation merge.`
			);
			return {};
		}
	}

	private async readPreservedKeys(): Promise<KeysFile> {
		return this.readKeysJsonFile(
			keyPreservationMirrorPath,
			'preserved mirror'
		);
	}

	private async pruneKeySnapshots(): Promise<void> {
		if (keySnapshotMaxFiles <= 0) return;

		try {
			const entries = await fs.promises.readdir(keySnapshotDir, {
				withFileTypes: true
			});
			const snapshots = entries
				.filter(
					entry =>
						entry.isFile() &&
						/^keys-\d{8}T\d{6}\d{3}Z\.json$/.test(entry.name)
				)
				.map(entry => entry.name)
				.sort()
				.reverse();

			const stale = snapshots.slice(keySnapshotMaxFiles);
			await Promise.all(
				stale.map(name =>
					unlinkFileCompat(path.join(keySnapshotDir, name)).catch(
						() => undefined
					)
				)
			);
			...
			if (err?.code !== 'ENOENT') {
				console.warn(
					'[DataManager] Failed pruning key preservation snapshots:',
					err
				);
			}
		}
	...
	private async writeKeyPreservationSnapshotIfNeeded(
		keys: KeysFile,
		reason: string,
		force = false
	): Promise<void> {
		if (keySnapshotIntervalMs <= 0 && !force) return;

		const now = Date.now();
		if (!force && now - lastKeySnapshotAt < keySnapshotIntervalMs) {
			return;
		}
		...
		try {
			...
			await writeFileAtomic(snapshotPath, JSON.stringify(keys, null, 2));
			lastKeySnapshotAt = now;
			await this.pruneKeySnapshots();
			console.log(
				`[DataManager] Wrote key preservation snapshot (${reason}) to ${snapshotPath}.`
			);
		} catch (err) {
			console.error(
				'[DataManager] Failed writing key preservation snapshot:',
				err
			);
		}
	}

	private async prepareKeysForWrite(
		keys: KeysFile,
		reason: string
	): Promise<KeysFile> {
		let merged = this.normalizeKeysSchema(keys || {}).normalized;
		const inputCount = this.countKeys(merged);

		if (!allowDestructiveKeySync) {
			const [filesystemKeys, preservedKeys] = await Promise.all([
				this.readKeysJsonFile(filePaths.keys, 'filesystem'),
				this.readPreservedKeys()
			]);

			const missingFromFilesystem = this.findMissingKeys(
				merged,
				filesystemKeys
			);
			const missingFromPreservation = this.findMissingKeys(
				merged,
				preservedKeys
			);
			...
			if (missingFromFilesystem.length > 0) {
				console.warn(
					`[DataManager] Key preservation restored ${this.describeKeySet(missingFromFilesystem)} from filesystem during ${reason}.`
				);
				merged = this.mergeKeysData(merged, filesystemKeys);
			}

			if (missingFromPreservation.length > 0) {
				console.warn(
					`[DataManager] Key preservation restored ${this.describeKeySet(missingFromPreservation)} from preservation mirror during ${reason}.`
				);
				merged = this.mergeKeysData(merged, preservedKeys);
			}
		}

		try {
			await ensureDir(keyPreservationDir);
			...
			console.error(
				'[DataManager] Failed writing key preservation mirror:',
				err
			);
		}

		await this.writeKeyPreservationSnapshotIfNeeded(
			merged,
			reason,
			inputCount < this.countKeys(merged)
		);

		return merged;
	}

	private normalizeProviderModelData(
		raw: Partial<ProviderModelData> | undefined,
		modelId: string
	): ProviderModelData {
		const responseTimes = Array.isArray(raw?.response_times)
			? raw!.response_times
			: [];
		const capabilitySkipsRaw = raw?.capability_skips;
		const capabilitySkips =
			capabilitySkipsRaw && typeof capabilitySkipsRaw === 'object'
				? Object.fromEntries(
						Object.entries(
							capabilitySkipsRaw as Record<string, unknown>
						)
							.filter(
								([_, value]) =>
				...
				: undefined;
		const disabled_at =
			typeof (raw as any)?.disabled_at === 'number' &&
			(raw as any).disabled_at > 0
				? (raw as any).disabled_at
				: undefined;
		const disable_count =
			typeof (raw as any)?.disable_count === 'number' &&
			(raw as any).disable_count >= 0
				? Math.floor((raw as any).disable_count)
				: undefined;
		const rateLimitRps =
			typeof (raw as any)?.rate_limit_rps === 'number' &&
			Number.isFinite((raw as any).rate_limit_rps)
				? Math.max(0, Number((raw as any).rate_limit_rps))
				: null;
		return {
			id: typeof raw?.id === 'string' && raw.id ? raw.id : modelId,
			token_generation_speed:
				typeof raw?.token_generation_speed === 'number' &&
				!Number.isNaN(raw.token_generation_speed)
					? raw.token_generation_speed
					: 50,
			response_times: responseTimes,
			errors:
				typeof raw?.errors === 'number' && !Number.isNaN(raw.errors)
					? Math.max(0, raw.errors)
					: 0,
			consecutive_errors:
				typeof raw?.consecutive_errors === 'number' &&
				!Number.isNaN(raw.consecutive_errors)
					? Math.max(0, raw.consecutive_errors)
					: 0,
			avg_response_time:
				typeof raw?.avg_response_time === 'number'
					? raw.avg_response_time
					: null,
			avg_provider_latency:
				typeof raw?.avg_provider_latency === 'number'
					? raw.avg_provider_latency
					: null,
			avg_token_speed:
				typeof raw?.avg_token_speed === 'number'
					? raw.avg_token_speed
					: null,
			rate_limit_rps: rateLimitRps,
			capability_skips:
				capabilitySkips && Object.keys(capabilitySkips).length
					? capabilitySkips
					: undefined,
	...
	private mergeProviderModelData(
		fsModel: Partial<ProviderModelData> | undefined,
		redisModel: Partial<ProviderModelData> | undefined,
		modelId: string
	): ProviderModelData {
		const normalizedFs = this.normalizeProviderModelData(fsModel, modelId);
		const normalizedRedis = this.normalizeProviderModelData(
			redisModel,
			modelId
		);
		const mergedSkips = {
		...
		const mergedRateLimitRps =
			normalizedRedis.rate_limit_rps ??
			normalizedFs.rate_limit_rps ??
			null;
		const preferredModel =
			dataSourcePreference === 'redis' ? normalizedRedis : normalizedFs;
		const fallbackModel =
			preferredModel === normalizedRedis ? normalizedFs : normalizedRedis;
		...
		return {
			...
			token_generation_speed: Math.max(
				1,
				normalizedRedis.token_generation_speed ||
					normalizedFs.token_generation_speed ||
					50
			),
			response_times: this.mergeResponseTimes(
				normalizedFs.response_times,
				normalizedRedis.response_times
			),
			errors: Math.max(normalizedFs.errors, normalizedRedis.errors),
			consecutive_errors: Math.max(
				normalizedFs.consecutive_errors,
				normalizedRedis.consecutive_errors
			),
			avg_response_time:
				normalizedRedis.avg_response_time ??
				normalizedFs.avg_response_time,
			avg_provider_latency:
				normalizedRedis.avg_provider_latency ??
				normalizedFs.avg_provider_latency,
			avg_token_speed:
				normalizedRedis.avg_token_speed ?? normalizedFs.avg_token_speed,
			rate_limit_rps: mergedRateLimitRps,
			capability_skips: Object.keys(mergedSkips).length
				? mergedSkips
				: undefined,
			disabled: mergedDisabled,
			...(mergedDisabledAt !== undefined
				? { disabled_at: mergedDisabledAt }
				: {}),
			...(mergedDisableCount !== undefined
				? { disable_count: mergedDisableCount }
				: {})
		};
	}

	private normalizeProviderData(
		raw: Partial<LoadedProviderData> | undefined,
		providerId: string
	): LoadedProviderData {
		...
		for (const [modelId, modelData] of Object.entries(rawModels)) {
			models[modelId] = this.normalizeProviderModelData(
				modelData as Partial<ProviderModelData>,
				modelId
			);
		}
		...
		const lastErrorRaw =
			typeof (raw as any)?.lastError === 'string'
				...
				: typeof (raw as any)?.last_error === 'string'
		...
		const lastErrorCodeRaw =
			typeof (raw as any)?.lastErrorCode === 'string'
				? (raw as any).lastErrorCode
				: typeof (raw as any)?.last_error_code === 'string'
					? (raw as any).last_error_code
					: undefined;
		const lastErrorCode =
			typeof lastErrorCodeRaw === 'string' &&
			lastErrorCodeRaw.trim().length > 0
				? lastErrorCodeRaw.slice(0, 200)
				: undefined;
		const lastStatusRaw =
			typeof (raw as any)?.lastStatus === 'number'
				? (raw as any).lastStatus
				: typeof (raw as any)?.last_status === 'number'
					? (raw as any).last_status
					: null;
		const lastStatus =
			typeof lastStatusRaw === 'number' &&
			Number.isFinite(lastStatusRaw) &&
			lastStatusRaw > 0
				? Math.floor(lastStatusRaw)
				: null;
		const lastErrorAtRaw =
			typeof (raw as any)?.lastErrorAt === 'number'
				? (raw as any).lastErrorAt
				: typeof (raw as any)?.last_error_at === 'number'
					? (raw as any).last_error_at
					: undefined;
		const lastErrorAt =
			typeof lastErrorAtRaw === 'number' &&
			Number.isFinite(lastErrorAtRaw) &&
			lastErrorAtRaw > 0
				? Math.floor(lastErrorAtRaw)
				: undefined;
		const disabledReasonRaw =
			typeof (raw as any)?.disabled_reason === 'string'
				? (raw as any).disabled_reason
				: typeof (raw as any)?.disabledReason === 'string'
					? (raw as any).disabledReason
					: undefined;
		const disabled_reason =
			typeof disabledReasonRaw === 'string' &&
			disabledReasonRaw.trim().length > 0
		...
		return {
			...
			provider_url:
				typeof raw?.provider_url === 'string' ? raw.provider_url : '',
			provider_urls:
				typeof raw?.provider_urls === 'object' && raw.provider_urls
					? (raw.provider_urls as { [modelId: string]: string })
					: undefined,
			streamingCompatible:
				typeof raw?.streamingCompatible === 'boolean'
					? raw.streamingCompatible
					: undefined,
			models,
			...
			errors:
				typeof raw?.errors === 'number' && !Number.isNaN(raw.errors)
					...
					: 0,
			provider_score:
				typeof raw?.provider_score === 'number'
					? raw.provider_score
					: null,
			...(lastError ? { lastError } : {}),
	...
	private mergeProvidersData(
		redisProviders: LoadedProviders,
		fsProviders: LoadedProviders
	): LoadedProviders {
		...
		for (const provider of fsProviders || []) {
			...
			fsMap.set(
				provider.id,
				this.normalizeProviderData(provider, provider.id)
			);
		}
		...
		for (const providerId of allProviderIds) {
			...
			const baseRedis = this.normalizeProviderData(
				redisProvider,
				providerId
			);
			const fsLastErrorAt =
				typeof baseFs.lastErrorAt === 'number' ? baseFs.lastErrorAt : 0;
			const redisLastErrorAt =
				typeof baseRedis.lastErrorAt === 'number'
					? baseRedis.lastErrorAt
					: 0;
			const latestErrorSource =
				redisLastErrorAt >= fsLastErrorAt ? baseRedis : baseFs;
			const fallbackErrorSource =
				latestErrorSource === baseRedis ? baseFs : baseRedis;
			const mergedLastErrorAt =
				Math.max(fsLastErrorAt, redisLastErrorAt) || undefined;
			...
			const mergedDisabledReason = mergedDisabled
				? preferredDisabledSource.disabled_reason ||
					secondaryDisabledSource.disabled_reason
				: undefined;
			const mergedErrorFields =
				mergedLastErrorAt !== undefined
					? latestErrorSource
					: {
							lastError:
								latestErrorSource.lastError ||
								fallbackErrorSource.lastError,
							lastErrorCode:
								latestErrorSource.lastErrorCode ||
								fallbackErrorSource.lastErrorCode,
							lastStatus:
								latestErrorSource.lastStatus ??
								fallbackErrorSource.lastStatus,
			...
			for (const modelId of modelIds) {
				mergedModels[modelId] = this.mergeProviderModelData(
					baseFs.models?.[modelId],
					baseRedis.models?.[modelId],
					modelId
				);
			}
			...
			merged.push({
				id: providerId,
				apiKey: hasFilesystemProvider
					? baseFs.apiKey
					: baseRedis.apiKey || null,
				provider_url:
					baseRedis.provider_url || baseFs.provider_url || '',
				provider_urls: baseRedis.provider_urls || baseFs.provider_urls,
				streamingCompatible:
					typeof baseRedis.streamingCompatible === 'boolean'
						? baseRedis.streamingCompatible
						: baseFs.streamingCompatible,
				models: mergedModels,
				disabled: mergedDisabled,
				avg_response_time:
					baseRedis.avg_response_time ?? baseFs.avg_response_time,
				avg_provider_latency:
					baseRedis.avg_provider_latency ??
					baseFs.avg_provider_latency,
				errors: Math.max(baseFs.errors || 0, baseRedis.errors || 0),
				provider_score:
					baseRedis.provider_score ?? baseFs.provider_score,
	...
	public async syncKeysBetweenRedisAndFilesystem(): Promise<void> {
		...
		if (this.isRedisReady()) {
			try {
				const redisRaw = await this.redisClient!.hget(
					redisHashKey,
					redisHashFields.keys
				);
				if (redisRaw) {
					redisKeys = JSON.parse(redisRaw) as KeysFile;
				} else {
					const legacyRaw = await this.redisClient!.get(
						legacyRedisKeys.keys
					);
					if (legacyRaw) {
				...
				console.error(
					'[DataManager] Failed reading keys from Redis for sync:',
					err
				);
			}
		}
		...
		const preservedKeys = await this.readPreservedKeys();
		const hasPreservedKeys = Object.keys(preservedKeys).length > 0;

		if (!hasRedisKeys && !hasFsKeys && !hasPreservedKeys) {
			console.warn(
				'[DataManager] Keys sync skipped: filesystem, Redis, and preservation mirror are empty.'
			);
			return;
		}

		if (fsInvalid && !hasRedisKeys && !hasPreservedKeys) {
			console.warn(
				'[DataManager] Keys sync skipped: filesystem keys are invalid and Redis/preservation mirror have no keys.'
			);
			return;
		}
				...
				dataSourcePreference === 'redis' && hasRedisKeys
					...
					: dataSourcePreference === 'filesystem' &&
						...
						: hasRedisKeys
							...
							: hasFsKeys && !fsInvalid
								? this.normalizeKeysSchema(fsKeys).normalized
								: this.normalizeKeysSchema(preservedKeys)
										.normalized;
		} else {
			merged =
				hasRedisKeys && hasFsKeys && !fsInvalid
					? this.mergeKeysData(redisKeys, fsKeys)
					: hasRedisKeys
						? this.normalizeKeysSchema(redisKeys).normalized
						: hasFsKeys && !fsInvalid
							? this.normalizeKeysSchema(fsKeys).normalized
		...
		if (hasPreservedKeys && !allowDestructiveKeySync) {
			merged = this.mergeKeysData(merged, preservedKeys);
		}

		merged = await this.prepareKeysForWrite(
			merged,
			'redis-filesystem-sync'
		);
		...
		try {
			...
			console.error(
				'[DataManager] Failed writing merged keys to filesystem:',
				err
			);
		}
		...
		if (this.isRedisReady()) {
			try {
				...
				console.error(
					'[DataManager] Failed writing merged keys to Redis:',
					err
				);
			}
		}
		...
		console.log(
			`[DataManager] Keys sync complete. Total keys: ${Object.keys(merged).length}`
		);
	}
	...
	public async syncProvidersBetweenRedisAndFilesystem(): Promise<void> {
		...
		try {
			...
			if (err?.code !== 'ENOENT') {
				console.error(
					'[DataManager] Failed reading providers from filesystem for sync:',
					err
				);
			}
		}
		...
		if (this.isRedisReady()) {
			try {
				const redisRaw = await this.redisClient!.hget(
					redisHashKey,
					redisHashFields.providers
				);
				if (redisRaw) {
					redisProviders = JSON.parse(redisRaw) as LoadedProviders;
				} else {
					const legacyRaw = await this.redisClient!.get(
						legacyRedisKeys.providers
					);
					if (legacyRaw) {
						redisProviders = JSON.parse(
							legacyRaw
						) as LoadedProviders;
					}
				}
				...
				console.error(
					'[DataManager] Failed reading providers from Redis for sync:',
					err
				);
			}
		}
		...
		try {
			...
			console.error(
				'[DataManager] Failed writing merged providers to filesystem:',
				err
			);
		}
		...
		if (this.isRedisReady()) {
			try {
				...
				console.error(
					'[DataManager] Failed writing merged providers to Redis:',
					err
				);
			}
		}
		...
		console.log(
			`[DataManager] Providers sync complete. Total providers: ${merged.length}`
		);
	}

	private normalizeKeysSchema(keys: KeysFile): {
		normalized: KeysFile;
		changed: boolean;
	} {
		let changed = false;
		...
		for (const [apiKey, rawUser] of Object.entries(keys || {})) {
			...
			if (
				typeof user.requestCount !== 'number' ||
				Number.isNaN(user.requestCount) ||
				user.requestCount < 0
			) {
				user.requestCount = 0;
				changed = true;
			...
			if (typeof user.dailyRequestCount !== 'undefined') {
				const normalizedDailyRequestCount = normalizeCounter(
			...
			if (
				typeof user.dailyRequestDate !== 'undefined' &&
				!isValidDayBucket(user.dailyRequestDate)
			) {
			...
			if (
				typeof user.dailyRequestCount !== 'undefined' &&
				typeof user.dailyRequestDate === 'undefined'
			) {
				user.dailyRequestDate = nowDayBucket;
				changed = true;
			...
			if (typeof user.estimatedCost !== 'undefined') {
				const normalizedEstimatedCost = normalizeCost(
					user.estimatedCost
				);
			...
			if (typeof user.disabled !== 'undefined') {
				const normalizedDisabled = Boolean(user.disabled);
				if (normalizedDisabled !== user.disabled) {
					user.disabled = normalizedDisabled;
					changed = true;
				}
			}
			...
			const totalEstimatedCost =
				typeof user.estimatedCost === 'number'
			...
			if (!hasExplicitBillingState) {
				...
				if (!isValidTimestamp(user.billingPeriodStartedAt)) {
					user.billingPeriodStartedAt = isValidTimestamp(
	...
	private async syncModelsMirrorIfNeeded(
		...
		if (source === 'redis') {
			try {
				...
				console.error(
					'[DataManager] Failed syncing models from Redis to filesystem:',
					err
				);
				return;
			}
			...
			try {
				await this.redisClient!.hset(
					redisHashKey,
					redisHashFields.models,
					JSON.stringify(modelsData)
				);
				await this.redisClient!.del(legacyRedisKeys.models);
			} catch (err) {
				console.error(
					'[DataManager] Failed syncing models from filesystem to Redis:',
					err
				);
				return;
			}
	...
	async runStartupRedisMigration(): Promise<void> {
		if (!this.isRedisReady()) {
			console.log(
				'[DataManager] Skipping startup Redis migration: Redis client not ready.'
			);
			return;
		}

		try {
			const alreadyMigrated = await this.redisClient!.hget(
				redisHashKey,
				redisMigrationMarkerField
			);
			if (alreadyMigrated === '1') {
				console.log(
					'[DataManager] Startup Redis migration already completed.'
				);
				return;
			}
			...
			for (const dataType of dataTypes) {
				...
				const existingHashValue = await this.redisClient!.hget(
					redisHashKey,
					redisField
				);
				if (!existingHashValue) {
					const legacyValue =
						await this.redisClient!.get(legacyRedisKey);
					if (legacyValue) {
						await this.redisClient!.hset(
							redisHashKey,
							redisField,
			...
			console.log(
				`[DataManager] Startup Redis migration complete. Migrated: ${migratedCount}, legacy keys removed: ${cleanedCount}.`
			);
		} catch (err) {
	...
	async load<T extends ManagedDataStructure>(dataType: DataType): Promise<T> {
		...
		const loadPromise = (async (): Promise<T> => {
			...
			const loadFromRedis = async (): Promise<T | null> => {
				if (!this.isRedisReady()) {
					redisLoadState.status = 'unavailable';
					if (!filesystemFallbackLogged.has(dataType)) {
						console.log(
							`DataManager: Redis not ready, cannot load ${dataType} from Redis.`
						);
				...
				const cooldownUntil =
					this.redisReadCooldownUntil.get(dataType) || 0;
				if (cooldownUntil > Date.now()) {
					redisLoadState.status = 'cooldown';
					return null;
				}

				try {
					const redisTimeoutMs =
						dataType === 'keys'
							? keysRedisReadTimeoutMs
							: redisReadTimeoutMs;
					...
					if (redisData) {
						...
						redisLoadState.status = 'success';
						console.log(
							`[DataManager] Data loaded successfully from Redis for: ${dataType}`
						);
					...
					if (legacyData) {
						...
						redisLoadState.status = 'success';
						console.log(
							`[DataManager] Legacy Redis key found for ${dataType}; migrating to hash namespace.`
						);
						await this.redisClient!.hset(
							redisHashKey,
							redisField,
					...
					redisLoadState.status = 'empty';
					console.log(
						`[DataManager] No data found in Redis for: ${dataType}`
					);
					return null; // Explicitly return null if key doesn't exist in Redis
				} catch (err) {
					redisLoadState.status = 'error';
					if (redisFailureCooldownMs > 0) {
						this.redisReadCooldownUntil.set(
							dataType,
							Date.now() + redisFailureCooldownMs
						);
					}
					console.error(
						`[DataManager] Error loading/parsing from Redis for ${dataType}. Error:`,
						err
					);
					return null;
				}
			...
			const loadFromFilesystem = async (): Promise<T | null> => {
				if (!filesystemFallbackLogged.has(dataType)) {
					...
					if (
						...
						console.log(
							`DataManager: Loading ${dataType} from filesystem.`
						);
				...
				try {
					const fileData = await readTextFile(filePath);
					console.log(
						`[DataManager] Data loaded successfully from Filesystem for: ${dataType}`
					);
					return JSON.parse(fileData) as T;
				} catch (err: any) {
					if (err?.code === 'ENOENT') {
						console.warn(
							`[DataManager] File not found: ${filePath}. Cannot load ${dataType} from filesystem.`
						);
						return null;
					}
					if (dataType === 'keys') {
					...
					console.error(
						`[DataManager] Error loading/parsing file ${filePath}. Error:`,
						err
					);
					return null;
				}
			...
			if (dataSourcePreference === 'redis') {
				...
				if (loadedData === null) {
					// If Redis failed or was empty, try filesystem
					console.log(
						`[DataManager] Redis preferred, but failed/empty for ${dataType}. Trying filesystem.`
					);
				...
				if (loadedData === null) {
					// If filesystem failed or was empty, try Redis
					console.log(
						`[DataManager] Filesystem preferred, but failed/empty for ${dataType}. Trying Redis.`
					);
					loadedData = await loadFromRedis();
					if (loadedData !== null) {
						loadedFromFallback = true; // Mark that data was loaded from fallback
			...
			if (loadedData !== null) {
				if (dataType === 'keys') {
					const { normalized, changed } = this.normalizeKeysSchema(
						loadedData as KeysFile
					);
					if (changed) {
						...
						try {
							if (
								loadedSource === 'filesystem' &&
								dataSourcePreference === 'redis' &&
								redisLoadState.status !== 'empty'
							) {
								await this.saveToFilesystemOnly(
									'keys',
									loadedData
								);
							} else {
							...
							console.error(
								'[DataManager] Failed persisting migrated keys schema:',
								err
							);
						}
					}
				...
				if (dataType === 'models' && loadedSource) {
					await this.syncModelsMirrorIfNeeded(
						loadedData as ModelsFileStructure,
						loadedSource
					);
				}
				...
				if (loadedFromFallback) {
					const canSyncFallbackToPreferred =
						dataType !== 'keys' ||
						dataSourcePreference !== 'redis' ||
						redisLoadState.status === 'empty';
					if (!canSyncFallbackToPreferred) {
						console.warn(
							`[DataManager] Data for ${dataType} loaded from filesystem fallback after Redis ${redisLoadState.status}; ` +
								'skipping Redis write-back to avoid overwriting newer live data.'
						);
					} else {
						console.log(
							`[DataManager] Data for ${dataType} loaded from fallback. Attempting to save back to synchronize sources.`
						);
						if (
							dataSourcePreference === 'redis' &&
							!this.isRedisReady()
						...
						try {
							...
							this.save(dataType, loadedData).catch(saveErr => {
								console.error(
									`[DataManager] Asynchronous save after fallback for ${dataType} failed:`,
									saveErr
								);
							});
						} catch (saveErr) {
							...
							console.error(
								`[DataManager] Error initiating save for ${dataType} back after fallback load:`,
								saveErr
							);
						}
					}
			...
			console.warn(
				`[DataManager] Data for ${dataType} not found in ${dataSourcePreference} or fallback source. Using/creating default.`
			);
			...
			try {
				...
				console.error(
					`[DataManager] CRITICAL: Failed to save default value for ${dataType} during initial load. Error:`,
					saveErr
				);
			}
		...
		this.inFlightLoads.set(
			dataType,
			loadPromise as Promise<ManagedDataStructure>
		);
		try {
	...
	async updateWithLock<T extends ManagedDataStructure>(
		...
		const effectiveMaxRetries =
			dataType === 'providers' ? Math.max(maxRetries, 8) : maxRetries;
		...
		if (this.isRedisReady()) {
			for (let attempt = 0; attempt < effectiveMaxRetries; attempt++) {
				...
				try {
					...
					let currentRaw = await txClient.hget(
						redisHashKey,
						redisField
					);
					if (!currentRaw) {
					...
					if (dataType === 'keys') {
						currentData = this.normalizeKeysSchema(
							currentData as KeysFile
						).normalized as T;
					}
					...
					if (dataType === 'keys') {
						updated = this.normalizeKeysSchema(updated as KeysFile)
							.normalized as T;
						updated = (await this.prepareKeysForWrite(
							updated as KeysFile,
							'redis-update-with-lock'
						)) as T;
					}
					...
					if (execResult !== null) {
						inMemoryCache[dataType] = {
							value: updated,
							ts: Date.now()
						};
						// Mirror Redis updates back to filesystem to keep local JSON in sync.
						void this.saveToFilesystemOnly(
							dataType,
							updated,
							dataType === 'keys'
						).catch(err => {
							console.warn(
								`[DataManager] Failed filesystem mirror after Redis update for ${dataType}:`,
								err
							);
						});
						return updated;
					...
					if (attempt === effectiveMaxRetries - 1) {
						console.warn(
							`[DataManager] Redis updateWithLock attempt ${attempt + 1} failed for ${dataType}:`,
							err
						);
					}
				} finally {
			...
			console.warn(
				`[DataManager] Falling back to filesystem update for ${dataType} after Redis contention.`
			);
		}
	...
	private async saveToFilesystemOnly<T extends ManagedDataStructure>(
		dataType: DataType,
		data: T,
		keysAlreadyPreserved = false
	): Promise<void> {
		...
		if (
			...
			console.error(
				'[DataManager] Refusing to save providers.json because data array is empty. Aborting save to prevent destruction.'
			);
			return;
		}

		if (dataType === 'keys' && !keysAlreadyPreserved) {
			data = (await this.prepareKeysForWrite(
				data as KeysFile,
				'filesystem-only-save'
			)) as T;
		}

		const filesystemSyncIntervalMs = getFilesystemSyncIntervalMs(dataType);
		if (filesystemSyncIntervalMs > 0) {
			const filePath = filePaths[dataType];
			...
			try {
				...
				console.error(
					`[DataManager] Error stringifying data for ${dataType} (filesystem mirror). Aborting save. Error:`,
					err
				);
				return;
			}
			...
			const timer = setTimeout(async () => {
				...
				try {
					await writeFileAtomic(
						latestPending.filePath,
						latestPending.data
					);
				} catch (writeErr) {
					console.error(
						`[DataManager] Failed writing ${dataType} to filesystem mirror (delayed):`,
						writeErr
					);
				} finally {
					pendingFsWrites.delete(dataType);
				}
			}, filesystemSyncIntervalMs);
		...
		try {
			...
			console.error(
				`[DataManager] Failed writing ${dataType} to filesystem mirror:`,
				err
			);
		}
	}

	async save<T extends ManagedDataStructure>(
		dataType: DataType,
		data: T
	): Promise<void> {
		// Safety guard: never overwrite providers.json with an empty array
		...
		if (dataType === 'keys' && dataSourcePreference === 'redis') {
			...
			await this.saveToFilesystemOnly(dataType, data, true);
			return;
		}
		...
		try {
			...
			console.error(
				`[DataManager] Error stringifying data for ${dataType}. Aborting save. Error:`,
				err
			);
		...
		const redisSavePromise = (async () => {
			if (!this.isRedisReady()) {
				if (!filesystemFallbackLogged.has(dataType)) {
					console.log(
						`DataManager: Redis not ready. Cannot save ${dataType} to Redis.`
					);
				}
				if (dataSourcePreference === 'redis') {
					console.error(
						`[DataManager] CRITICAL: Cannot save to preferred source (Redis) for ${dataType} - Client not ready.`
					);
				}
				return;
			}
			...
			try {
				...
				redisSuccess = true;
				console.log(
					`[DataManager] Data saved successfully to Redis for: ${dataType}`
				);
			} catch (err) {
				console.error(
					`[DataManager] Error saving to Redis hash for ${dataType}. Error:`,
					err
				);
				if (dataSourcePreference === 'redis') {
					console.error(
						`[DataManager] CRITICAL: Failed to save to preferred source (Redis) for ${dataType}.`
					);
				}
			}
		})();
		...
		const fsSavePromise = (async () => {
			try {
				if (getFilesystemSyncIntervalMs(dataType) > 0) {
					await this.saveToFilesystemOnly(
						dataType,
						data,
						dataType === 'keys'
				...
				fsSuccess = true;
				console.log(
					`[DataManager] Data saved successfully to Filesystem for: ${dataType}`
				);
			} catch (err) {
				console.error(
					`[DataManager] Error saving to file ${filePath}. Error:`,
					err
				);
				if (dataSourcePreference === 'filesystem') {
					console.error(
						`[DataManager] CRITICAL: Failed to save to preferred source (Filesystem) for ${dataType}.`
					);
				}
			}
		})();
		...
		if (!redisSuccess && !fsSuccess) {
			console.error(
				`[DataManager] !!! Data Save FAILED for ${dataType} on BOTH Redis and Filesystem !!!`
			);
		} else if (!redisSuccess && dataSourcePreference === 'redis') {
			console.warn(
				`[DataManager] WARNING: Saved ${dataType} to Filesystem, but FAILED to save to preferred source (Redis).`
			);
		} else if (!fsSuccess && dataSourcePreference === 'filesystem') {
			console.warn(
				`[DataManager] WARNING: Saved ${dataType} to Redis, but FAILED to save to preferred source (Filesystem).`
			);
		}
	}
}
...
export function isDataManagerReady(): boolean {
