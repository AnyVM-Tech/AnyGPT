        ...
        for (const provider of providersData) {
            ...
            for (const modelId in provider.models) {
                ...
                const providerConstraintEntries = Array.isArray((provider as any)?.capability_blocked)
                    ...
                    : [];
                const providerRemovedEntries = Array.isArray((provider as any)?.model_removed)
                    ? (provider as any).model_removed
                    : [];
                const providerSkipEntries = Array.isArray((provider as any)?.probe_skip)
                    ? (provider as any).probe_skip
                    : [];
                const providerHasAvailabilityConstraint = [
                    ...providerConstraintEntries,
                    ...providerRemovedEntries,
                    ...providerSkipEntries,
                ].some((entry: any) => {
