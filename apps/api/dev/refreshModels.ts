...
const REFRESH_MODELS_VALIDATION = 'Next validation success condition: at least one fresh same-thread api-data refresh/smoke run for this iteration confirms constrained Gemini image-output entries remain preserved in source-of-truth output, or a clear operator-facing defer reason explains why no run was emitted.';
...
async function main(): Promise<void> {
  ...
  if (!preservesPairedRegionalRemovalHandling || !preservesCountryOrRegionalConstraintReason) {
    const availabilityConstraintError = '[refreshModels] Availability constraint failure: lyria-3-pro-preview was not preserved with explicit Gemini country/provider-region image-output availability constraints after refresh. Review source-of-truth availability handling before treating this refresh as successful.';
