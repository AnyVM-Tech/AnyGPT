...
function normalizeResponsesFunctionItemId(
	rawId: unknown,
	callId?: string
): string {
...
export class OpenAI implements IAIProvider {
	...
	private recordUnsupportedParams(
		modelId: string,
		params: string[]
	): string[] {
	...
	private stripUnsupportedParamsFromPayload(
		...
		const unsupported = new Set<string>(
			this.getUnsupportedParamSet(modelId)
		);
	...
	private extractUnsupportedParams(error: any): string[] {
		...
		const paramField =
			error?.response?.data?.error?.param || error?.response?.data?.param;
		...
		if (results.size === 0) {
			const listMatch = message.match(
				/Unsupported parameters?:\s*([^\.\[]+)/i
			);
			if (listMatch) {
				const entries = listMatch[1].split(',');
				for (const entry of entries) {
					const normalized =
						this.normalizeUnsupportedParamName(entry);
					if (normalized) results.add(normalized);
				}
			}
		}
	...
	constructor(apiKey: string, endpointUrl?: string) {
		...
		if (!apiKey && !endpointUrl) {
			throw new Error(
				'Either an OpenAI API key or an endpoint URL must be provided'
			);
		}
		...
		if (apiKey && apiKey.startsWith('sk-')) {
			...
			if (endpointUrl) {
				...
				throw new Error(
					'Endpoint URL must be provided if API key is not an OpenAI API key'
				);
			}
		}
	...
	private getRequestTimeoutMs(): number {
		const upstreamTimeout = this.readEnvNumber(
			'UPSTREAM_TIMEOUT_MS',
			30_000
		);
	...
	private shouldUseResponsesApi(modelId: string, force?: boolean): boolean {
		...
		if (normalized.includes('o3') || normalized.includes('omni'))
			return true;
	...
	private resolveEndpoint(modelId: string, forceResponses?: boolean): string {
		const wantsResponses = this.shouldUseResponsesApi(
			modelId,
			forceResponses
		);
		return wantsResponses
			? this.resolveResponsesEndpoint()
	...
	private normalizeChatMessages(message: IMessage) {
		...
		return source.map(entry => {
			...
			if (
				Array.isArray((entry as any).tool_calls) &&
				(entry as any).tool_calls.length > 0
			) {
				normalized.tool_calls = (entry as any).tool_calls;
			}
			if (
				typeof (entry as any).tool_call_id === 'string' &&
				(entry as any).tool_call_id.trim()
			) {
	...
	private buildChatStreamOptions(
		message: IMessage
	): Record<string, any> | undefined {
		const existing =
			message.stream_options && typeof message.stream_options === 'object'
				? { ...message.stream_options }
				: {};

		if (
			this.hasCustomEndpoint &&
			Object.prototype.hasOwnProperty.call(existing, 'include_usage')
		) {
	...
	private buildResponsesStreamOptions(
		message: IMessage
	): Record<string, any> | undefined {
		const existing =
			message.stream_options && typeof message.stream_options === 'object'
	...
	private attachChatOptionalParams(
		...
		if (typeof message.tools !== 'undefined') {
			target.tools =
				this.normalizeChatTools(message.tools) ?? message.tools;
		}
		if (typeof message.tool_choice !== 'undefined') {
			target.tool_choice = this.normalizeChatToolChoice(
				message.tool_choice
			);
		}
		if (
			typeof message.reasoning !== 'undefined' &&
			this.supportsReasoningParam(message.model.id)
		) {
			target.reasoning = message.reasoning;
		}
		if (
			typeof message.service_tier === 'string' &&
			message.service_tier.trim()
		) {
		...
		const hasAudioModality =
			Array.isArray(message.modalities) &&
			message.modalities.some(
				modality => String(modality).toLowerCase() === 'audio'
			);
		if (
			this.isAudioModel(message.model.id) &&
			!hasAudioInput &&
			!hasAudioModality
		) {
	...
	private normalizeChatToolChoice(toolChoice: any): any {
		...
		if (name) {
			return {
				type: (toolChoice as any).type ?? 'function',
				function: { name }
			};
		}
		return toolChoice;
	...
	private normalizeResponsesTools(tools: any): any[] | undefined {
		...
		return tools.map(tool => {
			...
			if (func && typeof func === 'object') {
				...
				const description =
					func.description ?? (tool as any).description;
				...
				if (typeof (tool as any).strict !== 'undefined')
					normalized.strict = (tool as any).strict;
				if (
					typeof (func as any).strict !== 'undefined' &&
					typeof normalized.strict === 'undefined'
				) {
	...
	private normalizeResponsesToolChoice(toolChoice: any): any {
		...
		if (func && typeof func === 'object') {
			...
			if (name)
				return { type: (toolChoice as any).type ?? 'function', name };
	...
	private hasAudioInputContent(content: IMessage['content']): boolean {
		...
		for (const part of content) {
			...
			if (
				...
				typeof (part as any).input_audio?.data === 'string'
			)
				return true;
		}
	...
	private attachResponsesOptionalParams(
		target: Record<string, any>,
		message: IMessage
	) {
		...
		if (
			typeof message.max_tokens === 'number' &&
		...
		if (typeof message.tools !== 'undefined') {
			target.tools =
				this.normalizeResponsesTools(message.tools) ?? message.tools;
		}
		if (typeof message.tool_choice !== 'undefined') {
			target.tool_choice = this.normalizeResponsesToolChoice(
				message.tool_choice
			);
		}
		if (
		...
		if (message.audio) target.audio = message.audio;
		if (
			this.isComputerUseModel(message.model.id) &&
			typeof target.truncation === 'undefined'
		) {
	...
	private normalizeResponsesContentParts(content: any): any[] {
		...
		for (const part of parts) {
			...
			if (
				(type === 'input_image' || type === 'image_url') &&
				part.image_url
			) {
	...
	private normalizeResponsesSpecialInputItem(entry: any): any | null {
		...
		if (type === 'function_call') {
				...
				typeof entry.call_id === 'string' && entry.call_id.trim()
					...
					: typeof entry.tool_call_id === 'string' &&
						...
						: typeof entry.id === 'string' &&
							  entry.id.trim().startsWith('call_')
							? entry.id.trim()
							: undefined;
			const name =
			...
			const normalized: Record<string, any> = {
				id: normalizeResponsesFunctionItemId(entry.id, callId),
				type: 'function_call',
				name,
	...
	private normalizeResponsesToolCallInputItem(raw: any): any | null {
		...
		const callIdRaw =
			raw?.call_id ?? raw?.id ?? raw?.tool_call_id ?? raw?.tool_call?.id;
		const callId =
			typeof callIdRaw === 'string' && callIdRaw.trim()
				? callIdRaw.trim()
				: undefined;

		const normalized: Record<string, any> = {
			id: normalizeResponsesFunctionItemId(raw?.id, callId),
			type: 'function_call',
			name,
			arguments: this.stringifyResponsesValue(
	...
	private normalizeResponsesInput(rawInput: any): any[] {
		if (Array.isArray(rawInput) && rawInput.length > 0) {
			...
			for (const entry of rawInput) {
				...
				if (role) {
					...
					if (
						...
						const toolOutputItem =
							this.normalizeResponsesToolOutputInputItem(entry);
						if (toolOutputItem) {
							normalizedItems.push(toolOutputItem);
						} else if (
							this.hasMeaningfulResponsesContent(entry.content)
						) {
							normalizedItems.push({
								role: 'assistant',
								content: this.normalizeResponsesContentParts(
									entry.content
								),
					...
					if (
						...
						for (const toolCall of entry.tool_calls) {
							const functionCallItem =
								this.normalizeResponsesToolCallInputItem(
									toolCall
								);
							if (functionCallItem)
								normalizedItems.push(functionCallItem);
						}
					}
					continue;
				}
				...
				const specialItem =
					this.normalizeResponsesSpecialInputItem(entry);
				if (specialItem) {
					flushBufferedUserContent();
				...
				const normalizedParts =
					this.normalizeResponsesContentParts(entry);
			...
			return [
				{
					...
					content: [
						{
							type: 'input_text',
							text: this.normalizeContent(rawInput)
						}
					]
				}
			];
	...
	private buildChatPayload(message: IMessage, stream: boolean) {
		...
		if (
			...
			const requestedFormat =
				typeof payload.audio.format === 'string'
					? payload.audio.format.toLowerCase()
					: '';
			if (stream) {
	...
	private buildResponsesPayload(message: IMessage, stream: boolean) {
		...
		const hasStatefulInput =
			Boolean(message.previous_response_id) ||
			normalizedInput.some((entry: any) =>
				this.hasStatefulResponsesInputEntry(entry)
			);
		...
		normalizedInput = normalizedInput.map((entry: any) => {
			...
			const content = entry.content.map((part: any) => {
				...
				if (type === 'output_text' && typeof part.text === 'string')
					return part;
				if (
					type === 'output_text' &&
					typeof (part as any).output_text === 'string'
		...
		if (
			...
			payload.previous_response_id = message.previous_response_id.trim();
		}
		if (stream) {
	...
	private extractAudioFromPart(part: any): string | null {
		...
		const audioObj =
			part?.audio ?? part?.output_audio ?? part?.audio_output ?? null;
		const data =
			audioObj?.data ||
			audioObj?.b64_json ||
			audioObj?.base64 ||
	...
	private extractMediaFromResponsesOutput(output: any): string | null {
		...
		for (const entry of list) {
			...
			const direct =
				this.extractImageFromPart(entry) ||
				this.extractAudioFromPart(entry);
	...
	private extractResponsesToolCalls(data: any): any[] | undefined {
		...
		if (
			Array.isArray(data?.response?.tool_calls) &&
			data.response.tool_calls.length > 0
		)
			return data.response.tool_calls;

		const outputList = Array.isArray(data?.output)
			? data.output
			: data?.output
				? [data.output]
				: [];
		...
		if (data?.response?.output_item)
			candidates.push(data.response.output_item);
	...
	private extractToolCalls(
		...
		return useResponsesApi
			? this.extractResponsesToolCalls(data)
			: this.extractChatToolCalls(data);
	}
	...
	private extractReasoningText(raw: any): string | undefined {
		...
		if (typeof raw.reasoning === 'string')
			return raw.reasoning.trim() || undefined;
		if (typeof raw.reasoning_content === 'string')
			return raw.reasoning_content.trim() || undefined;
		return undefined;
	}
	...
	private extractResponseText(
		...
		if (useResponsesApi) {
			const media =
				this.extractMediaFromResponsesOutput(data?.output) ||
			...
			if (
				...
				data.response.output_text.length > 0
			)
		...
		if (Array.isArray(raw)) {
			...
			for (const part of raw) {
				...
				if (
					...
					part.transcript.length > 0
				)
					return part.transcript;
				if (
					typeof part.output_text === 'string' &&
					part.output_text.length > 0
				)
					return part.output_text;
				if (
					part.audio &&
					typeof part.audio.transcript === 'string' &&
					part.audio.transcript.length > 0
				)
					return part.audio.transcript;
			}
		}

		const audioTranscript = message?.audio?.transcript;
		if (typeof audioTranscript === 'string' && audioTranscript.length > 0)
			return audioTranscript;

		// Audio-only outputs may omit text fields; avoid failing the whole provider attempt.
		if (message?.audio || Array.isArray(raw))
			return '[Audio output generated]';
	...
	private extractResponsesStreamChunk(parsed: any): string {
		...
		const media =
			this.extractMediaFromResponsesOutput(parsed?.output) ||
			this.extractMediaFromResponsesOutput(parsed?.response?.output) ||
			this.extractMediaFromParts(
				Array.isArray(parsed?.content) ? parsed.content : []
			);
		if (media) return media;

		const audio = this.extractAudioFromPart(
			parsed?.audio ?? parsed?.output_audio ?? parsed
		);
		if (audio) return audio;
	...
	private extractChatStreamChunk(parsed: any): string {
		...
		const media =
			this.extractMediaFromParts(
				Array.isArray(delta?.content) ? delta.content : []
			) ||
	...
	async sendMessage(message: IMessage): Promise<ProviderResponse> {
		...
		const forcedResponses =
			this.hasCustomEndpoint &&
			this.isResponsesEndpoint(this.endpointUrl);
		const useResponsesApi = this.shouldUseResponsesApi(
			message.model.id,
			forcedResponses || message.useResponsesApi
		);
		...
		const stripped = this.stripUnsupportedParamsFromPayload(
			data,
			message.model.id
		);
		...
		try {
			...
			const missingInput =
				useResponsesApi && this.isMissingInputError(error);
			const missingMessages =
				!useResponsesApi && this.isMissingMessagesError(error);

			if (
				useResponsesApi &&
				(error?.response?.status === 400 || missingInput)
			) {
				console.error(
					'[OpenAI][Responses] 400 payload summary:',
					this.summarizeResponsesPayload(requestData)
				);
			}

			const unsupportedParams = this.extractUnsupportedParams(error);
			if (unsupportedParams.length > 0) {
				this.recordUnsupportedParams(
					message.model.id,
					unsupportedParams
				);
				const retried = this.stripUnsupportedParamsFromPayload(
					requestData,
					message.model.id,
					unsupportedParams
				);
				if (retried.removed.length > 0) {
					try {
						...
						const retryResp = await axios.post(
							url,
							retried.cleaned,
							{ headers, timeout: this.getRequestTimeoutMs() }
						);
						const retryLatency = Date.now() - retryStart;
						const retryText = this.extractResponseText(
							retryResp.data,
							useResponsesApi
						);
						if (retryText !== null && retryText !== undefined) {
							return {
								...
								reasoning: useResponsesApi
									? this.extractResponsesReasoningText(
											retryResp.data
										)
									: this.extractChatReasoning(retryResp.data)
							};
						}
					} catch (retryError: any) {
						console.error(
							'Retry after stripping unsupported params failed:',
							retryError
						);
					}
				}
			...
			if (
				...
				try {
					...
					const chatStripped = this.stripUnsupportedParamsFromPayload(
						chatPayload,
						message.model.id
					);
					...
					const chatResp = await axios.post(
						chatUrl,
						chatStripped.cleaned,
						{ headers, timeout: this.getRequestTimeoutMs() }
					);
					const chatLatency = Date.now() - chatStart;
					const chatText = this.extractResponseText(
						chatResp.data,
						false
					);
					...
					console.error(
						'Chat fallback after responses API 400 failed:',
						fallbackError
					);
				}
			}
			if (!useResponsesApi && missingMessages) {
				try {
					...
					const responsesPayload = this.buildResponsesPayload(
						message,
						false
					);
					const responsesStripped =
						this.stripUnsupportedParamsFromPayload(
							responsesPayload,
							message.model.id
						);
					...
					const responsesResp = await axios.post(
						responsesUrl,
						responsesStripped.cleaned,
						{ headers, timeout: this.getRequestTimeoutMs() }
					);
					const responsesLatency = Date.now() - responsesStart;
					const responsesText = this.extractResponseText(
						responsesResp.data,
						true
					);
					...
					console.error(
						'Responses fallback after chat missing messages failed:',
						fallbackError
					);
				}
			}
			...
			console.error(
				`Error during sendMessage to ${url} (Latency: ${latency}ms):`,
				{
	...
	async createPassthroughStream(
		message: IMessage
	): Promise<ProviderStreamPassthrough | null> {
	...
	async *sendMessageStream(
		...
		try {
			const response = await this.postSseRequest(
				url,
				requestData,
				headers
			);
			...
			for await (const value of response.data) {
				...
				for (const dataMessage of dataMessages) {
					...
					try {
						...
						const reasoning = useResponsesApi
							? this.extractResponsesReasoningDelta(parsed)
							: this.extractChatReasoning(parsed);
						if (useResponsesApi) {
			...
			if (
				...
				console.error(
					'[OpenAI][Responses][Stream] 400 payload summary:',
					this.summarizeResponsesPayload(requestData)
				);
			}
			...
			if (unsupportedParams.length > 0) {
				...
				if (retried.removed.length > 0) {
					try {
						const retryResp = await this.postSseRequest(
							url,
							retried.cleaned,
							headers
						);
						...
						for await (const value of retryResp.data) {
							...
							for (const dataMessage of dataMessages) {
								...
								try {
									...
									const reasoning = useResponsesApi
										? this.extractResponsesReasoningDelta(
												parsed
											)
										: this.extractChatReasoning(parsed);
									if (useResponsesApi) {
										...
										chunk =
											this.extractResponsesStreamChunk(
												parsed
											);
									} else {
										chunk =
											this.extractChatStreamChunk(parsed);
									}
									fullResponse += chunk;
									const latency = Date.now() - startTime;
									yield {
									...
									console.error(
										'Error parsing retry stream chunk:',
										retryParseError
									);
								}
							}
						...
						console.error(
							'Retry stream after stripping unsupported params failed:',
							retryError
						);
					}
				}
			...
			if (
				...
				try {
					...
					const chatResp = await this.postSseRequest(
						chatUrl,
						chatStripped.cleaned,
						headers
					);
					...
					for await (const value of chatResp.data) {
						...
						for (const dataMessage of dataMessages) {
							...
							try {
								...
								console.error(
									'Error parsing chat fallback stream chunk:',
									parseErr
								);
							}
						}
					...
					console.error(
						'Chat stream fallback after responses API 400 failed:',
						fallbackError
					);
				}
			}
			if (!useResponsesApi && missingMessages) {
				try {
					...
					const responsesResp = await this.postSseRequest(
						responsesUrl,
						responsesStripped.cleaned,
						headers
					);
					...
					for await (const value of responsesResp.data) {
						...
						for (const dataMessage of dataMessages) {
							...
							try {
								...
								const chunk =
									this.extractResponsesStreamChunk(parsed);
								fullResponse += chunk;
								const latency = Date.now() - startTime;
								yield {
									chunk,
								...
								console.error(
									'Error parsing responses fallback stream chunk:',
									parseErr
								);
							}
						}
					...
					console.error(
						'Responses stream fallback after chat missing messages failed:',
						fallbackError
					);
				}
			}
			...
			console.error(
				`Error during sendMessageStream to ${url} (Latency: ${latency}ms):`,
				{
					message: error?.message,
					status: error?.response?.status,
					data: error?.response?.data
			...
			const errorMessage = String(
				error?.message || this.extractApiErrorMessage(error)
			);
			const isEmptyStreamingResponse = /empty streaming response/i.test(
				errorMessage
			);
