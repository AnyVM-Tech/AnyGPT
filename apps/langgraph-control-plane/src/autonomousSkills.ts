...
const AUTONOMOUS_SKILLS: AutonomousSkillDefinition[] = [
  ...
  {
    id: 'anyscan-hardening',
    title: 'AnyScan Hardening',
    scopes: ['anyscan'],
    description: 'AnyScan work should stay on the Rust fetcher, Dragonfly/Redis persistence, Axum API surfaces, and worker orchestration with bounded cargo validation.',
    guidance: [
      'Prefer fetcher.rs, dragonfly_store.rs, config.rs, anyscan-api.rs, and anyscan-worker.rs over unrelated repo or API gateway edits.',
      'Bias toward concrete improvements in crawling, robots/sitemap discovery, request backoff, SSE/API behavior, job leasing, or persistence safety.',
      'Success means one bounded Rust/runtime improvement with cargo-level validation, not a speculative cross-app refactor.',
    ],
    references: [
      'apps/anyscan/Cargo.toml',
      'apps/anyscan/src/config.rs',
      'apps/anyscan/src/core.rs',
      'apps/anyscan/src/detectors.rs',
      'apps/anyscan/src/dragonfly_store.rs',
      'apps/anyscan/src/fetcher.rs',
      'apps/anyscan/src/store.rs',
      'apps/anyscan/src/bin/anyscan-api.rs',
      'apps/anyscan/src/bin/anyscan-worker.rs',
    ],
    signalPatterns: [
      'anyscan',
      'fetcher',
    ...
    contract: {
      summary: 'AnyScan contract: make one bounded Rust service improvement in crawling, persistence, API behavior, or worker orchestration with cargo validation.',
      checks: [
        'Prefer apps/anyscan Rust paths over unrelated API gateway or control-plane files.',
        'Success means one bounded change tied to fetcher, persistence, API, or worker behavior plus cargo validation.',
      ],
      preferredPaths: [
        'apps/anyscan/Cargo.toml',
        'apps/anyscan/src/config.rs',
